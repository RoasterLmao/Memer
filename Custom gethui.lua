...
local closure = newcclosure or new_cclosure or function(f) return f end
--[[ Script ]]--
local hui = Instance.new("Folder")
ProtectInstance(hui)
hui.Parent = game.CoreGui

getgenv().gethui = (function()
	return hui
end);

mt.__newindex = closure(function(self, prop, val)
   if self == hui then
       if prop == "Parent" or prop == "Name" or prop == "Archivable" then
         if prop == "Parent" then
      	      return old(self, "Parent", game.CoreGui)
         elseif prop == "Name" then
      	      return old(self, "Name", "Folder")
         elseif prop == "Archivable" then
      	      return old(self, "Archivale", false) -- no clone func
      	   end
   	end
   end
...
mt.__index = closure(function(self,prop,val)
   if self == hui and prop == "Parent" then
      return old2(self, "Parent", nil)
   end
   return old2(self,prop,val)
end)
